// ==========================================================================
// Home & Books Explore View (Security Hardened) - مِدادُ زَهْر
// ==========================================================================
...
export function renderHomeView() {
  ...
  const safeFeaturedCover = SecurityService.safeImageURL(featuredBook.cover);
  const safeFeaturedTitle = SecurityService.escapeHTML(featuredBook.title);
  const safeFeaturedAuthor = SecurityService.escapeHTML(featuredBook.author);
  const safeFeaturedDesc = SecurityService.escapeHTML(featuredBook.synopsis);
      ...
      <div class="container hero-grid">
        ...
        ${featuredBook.id ? `
          <div class="hero-featured-card">
            <img src="${safeFeaturedCover}" alt="${safeFeaturedTitle}" class="featured-cover" />
            <div class="featured-info">
              <span class="book-category-tag">${i18n.t('recommended')}</span>
              <h3>${safeFeaturedTitle}</h3>
              <div class="featured-author">${safeFeaturedAuthor}</div>
              <p class="featured-desc">${safeFeaturedDesc}</p>
...
function renderBookCard(book, state) {
  ...
  const safeCover = SecurityService.safeImageURL(book.cover);
  const safeTitle = SecurityService.escapeHTML(book.title);
  const safeAuthor = SecurityService.escapeHTML(book.author);
  const safeGenre = SecurityService.escapeHTML(book.genre || book.category);
  const safeBio = SecurityService.escapeHTML(book.synopsis);

  return `
    <div class="book-card" data-id="${book.id}">
      <div class="book-cover-wrap">
        <img src="${safeCover}" alt="${safeTitle}" class="book-cover-img" />
      ...
      <div class="book-card-body">
        <span class="book-category-tag">${safeGenre}</span>
        <h3 class="book-card-title" data-action="detail" data-id="${book.id}">${safeTitle}</h3>
        ...
        <p class="book-card-bio">${safeBio}</p>
